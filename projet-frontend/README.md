...
3. Créer la bdd et charger les données : 
```
php bin/console do:da:cr
php bin/console do:mi:mi -q
php bin/console do:fi:lo -q
```
4. Pour préparer l'environnement de test :
```
php bin/console do:da:cr --env=test
php bin/console do:mi:mi -q --env=test
php bin/console do:fi:lo -q --env=test
```
5. Lancer les tests avec `php bin/phpunit`
6. Lancer le projet avec `symfony server:start` ou `php -t -S public localhost:8000` pour lancer le serveur sur http://localhost:8000
